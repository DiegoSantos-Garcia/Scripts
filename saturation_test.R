...
option_list = list(
  make_option(c("-i", "--infolder"), action="store", default=".", type='character',
              help="Folder where alingments are stored. Default current directory."),
  make_option(c("-f", "--format"), action="store", default="*fasta.aln$", type='character',
              help="Format pattern present in alingments files. Default *fasta.aln"),
  make_option(c("-o", "--outfile"), action="store", default="saturation_matrix.tab",
              help="File name for regression coefficient values matrix and saturation plots. Default saturation_matrix.tab"),
